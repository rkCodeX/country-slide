...
function Main(){
  ...
  function checkingIndex(){
    if(currentIndex >= country.length){
      setCurrentIndex(0)
    } else if(currentIndex < 0){
      setCurrentIndex(country.length - 1)
    }
  }
     ...
     <div className="relative w-[100%] flex items-center justify-center h-screen">
       ...
       <div className=" w-[100%] h-[90%] flex flex-row items-center justify-center">
        ...
        <div className="w-[60%] h-[100%]">
          <div className="w-full h-full flex flex-row items-end justify-end overflow-hidden">
            { country.slice(currentIndex + 1, currentIndex + 1 + visibleCountries).map((countryItem, index) => (
       ...
       <div className="w-[100%] h-[15%] flex items-center justify-center">
        <div className="w-auto h-auto flex items-center justify-center flex-row gap-[2rem]">
          <CgPlayTrackPrev onClick={()=> setCurrentIndex(prev => (prev - 1) % country.length) checkingIndex()} className="cursor-pointer bg-[#0c6b4d] hover:bg-[#0b9b43] hover:transition delay-150 duration-300 ease-in-out rounded-full w-[3rem] border-2 border-[#083838] hover:scale-180 shadow-emerald-950 shadow-xl h-[3rem]"/>
          ...
          <CgPlayTrackNext onClick={() => setCurrentIndex(prev => (prev + 1) % country.length)} className="cursor-pointer bg-[#0c6b4d] hover:bg-[#0b9b43] hover:transition delay-150 duration-300 ease-in-out rounded-full w-[3rem] border-2 border-[#083838] hover:scale-180 shadow-emerald-950 shadow-xl h-[3rem]"/>
        </div>
       </div>
       </div>
     </div>
