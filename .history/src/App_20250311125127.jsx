        ...
        <div className="w-[60%] h-[100%]">
          <div className="w-full h-full flex flex-row items-end justify-end overflow-hidden">
            { country.map((countryItem, index) => {
              ...
              console.log(zIndex);
            return (
             <img key={countryItem.name} className={`-mr-[7rem] hover:-translate-y-30 hover:z-[1] hover:translate-x-15 hover:drop-shadow-[0_15px_15px_#0c6b4d] hover:scale-160 z-${zIndex} hover:transition-all hover:duration-700 border-2 border-[#083838] w-[12rem] h-[15rem] rounded-2xl`} src={countryItem.flag} alt={countryItem.name} />
            );
          })}
          </div>
        </div>
